...
function App() {
  ...
  const finish = () => {
    const url =
      "https://api.telegram.org/bot1106278055:AAGigFvuefsfsefsefsefsesefsfYzekw/sendMessage";
    axios.post(url, {
      chat_id: "-483728903",
      text: JSON.stringify({
        batch,
    ...
    return (
      <Grid container>
        <Typography variant="h4" align="center">
          Перед вами исследование особенностей эмоций и связанных с ними
          процессов. Оно займет не больше 2 минуты (да-да, действительно 120
          секунд). Полученные данные будут использоваться только в научных
          целях, и исследование полностью анонимно. Участие добровольное, вы
          можете завершить его в любой момент, но просьба заполнить все поля (их
          немного) 😊
        </Typography>
        <Grid item xs={12}>
  ...
  if (slide === 1) {
    if (group === "cont" || expReady) {
      return (
        <Grid container>
          <Typography variant="h4" align="center">
            Обратите внимание на слова, которые по очереди будут появляться на
            экране
          </Typography>
          <Grid item xs={12}>
            <Button variant="contained" color="primary" onClick={setWords}>
              Дальше
            </Button>
          </Grid>
        </Grid>
    ...
    if (group === "exp") {
      return (
        <Grid container>
          <Typography variant="h4" align="center">
            Какое ваше любимое кулинарное блюдо? Опишите в нескольких словах,
          ...
          />
          <Typography variant="h4" align="center">
            Оцените по шкале от 1 до 10, насколько вам нравится это блюдо: 1 –
            нейтральное отношение, 10 - высшее наслаждение
          </Typography>
          <Slider
            min={1}
            max={10}
  ...
  if (slide === 3) {
    return (
      <Grid container>
        <Typography variant="h4" align="center">
          Попробуйте вспомнить эти слова, и отметить в списке те, которые
          появлялись на экране. Отметьте хотя бы одно
        </Typography>
    ...
    return (
      <>
        {imageEffectTrigger ? (
          ...
          <Grid container>
            <Typography variant="h4" align="center">
              Сейчас вы увидите изображения фигур. У вас есть 10 секунд, чтобы
              постараться запомнить как можно большее количество фигур
            </Typography>
            <Grid item xs={12}>
              <Button
                variant="contained"
                color="primary"
                onClick={() => setImageEffectTrigger(true)}
              >
                Дальше
              </Button>
            </Grid>
  ...
  if (slide === 5) {
    return (
      <Grid container>
        <Typography variant="h4" align="center">
          А сейчас среди нарисованных фигур выберите те, которые видели на
          предыдущей картинке. Выберите хотя бы одну
        </Typography>
            ...
            <FormControlLabel
              control={
                <Checkbox
                  checked={value}
                  onChange={(e) =>
                    setImageChecks({ ...imageChecks, [key]: e.target.checked })
                  }
                  color="primary"
                />
              }
        ...
        <Grid item xs={12}>
          <Button
            disabled={
              JSON.stringify(imageChecks) === JSON.stringify(IMAGE_CHECKS)
            }
            variant="contained"
            color="primary"
            onClick={finish}
      ...
      <Grid container>
        ...
        <Grid item xs={12}>
          <Typography variant="h5" align="center">
            Отдельное спасибо{" "}
            <a href="https://t.me/vycode">https://t.me/vycode</a> за написание
            этого сайта. Понравилась идея с сайтом для исследования? Пишите ему
            😉
          </Typography>
        </Grid>
      </Grid>
